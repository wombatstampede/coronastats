 ...
 */
//modification -> get calling chart instance in callbacks before/afterZoom
...
function TracePlugin(Chart) {
	...
	var defaultOptions = {
		...
		callbacks: {
			beforeZoom: function(chartInstance, start, end) {
				return true;
			},
			afterZoom: function(chartInstance, start, end) {
			}
		}
	};
	...
	var crosshairPlugin = {
		...
		doZoom: function(chart, start, end) {
			...
			if (!beforeZoomCallback(chart, start, end)) {
				return false;
			}
			...
			afterZoomCallback(chart, start, end);
		},
